...
import { IfStatement } from "../melody-extension-core/types.js";
...
export default class Parser {
    ...
    matchAttributes(element, tokens) {
        while (
            ...
            if (tokens.test(Types.TAG_START)) {
                element.attributes.push(this.matchAttributeIfTag());
            } else {
                element.attributes.push(this.matchAttribute(tokens));
            }
        }
    }

    matchAttribute(tokens) {
        ...
        if (key) {
            ...
            return copyLoc(new n.Attribute(keyNode), keyNode);
            ...
            return new n.TwigComment(twigCommentValue);
        }
        this.error({
            title: "Invalid token",
            pos: tokens.la(0).pos,
            advice: "A tag must consist of attributes or expressions."
        });
    }

    /**
     * Matches an {% if %} ... {% endif %} block in attribute position,
     * e.g. <button {% if disabled %}disabled{% endif %}>.
     * The branch bodies consist of attributes instead of element children.
     */
    matchAttributeIfTag() {
        const tokens = this.tokens;
        const tagStartToken = tokens.expect(Types.TAG_START);
        const nameToken = tokens.expect(Types.SYMBOL);
        if (nameToken.text !== "if") {
            this.error({
                title: `Unexpected tag "${nameToken.text}"`,
                pos: nameToken.pos,
                advice: 'Only "if" tags are supported between element attributes.'
            });
        }

        const ifStatement = new IfStatement(this.matchExpression());
        tokens.expect(Types.TAG_END);
        ifStatement.consequent = this.matchAttributeIfBody();

        let branch = ifStatement;
        for (;;) {
            tokens.expect(Types.TAG_START);
            if (tokens.nextIf(Types.SYMBOL, "elseif")) {
                const elseIf = new IfStatement(this.matchExpression());
                tokens.expect(Types.TAG_END);
                elseIf.consequent = this.matchAttributeIfBody();
                branch.alternate = elseIf;
                branch = elseIf;
            } else if (tokens.nextIf(Types.SYMBOL, "else")) {
                tokens.expect(Types.TAG_END);
                branch.alternate = this.matchAttributeIfBody();
            } else if (tokens.nextIf(Types.SYMBOL, "endif")) {
                break;
            } else {
                this.error({
                    title: "Invalid token",
                    pos: tokens.la(0).pos,
                    advice: 'Expected "elseif", "else" or "endif".'
                });
            }
        }
        setStartFromToken(ifStatement, tagStartToken);
        setEndFromToken(ifStatement, tokens.expect(Types.TAG_END));
        return ifStatement;
    }

    matchAttributeIfBody() {
        const tokens = this.tokens;
        const attributes = [];
        for (;;) {
            if (tokens.test(Types.TAG_START)) {
                const next = tokens.la(1);
                if (next.type === Types.SYMBOL && next.text === "if") {
                    attributes.push(this.matchAttributeIfTag());
                    continue;
                }
                // elseif, else or endif — handled by matchAttributeIfTag
                return attributes;
            }
            if (
                tokens.test(Types.SLASH) ||
                tokens.test(Types.ELEMENT_END) ||
                tokens.test(Types.EOF)
            ) {
                this.error({
                    title: "Unterminated if tag",
                    pos: tokens.la(0).pos,
                    advice: 'An "if" tag inside an element must be closed with "endif" before the element ends.'
                });
            }
            if (tokens.nextIf(Types.EXPRESSION_START)) {
                // Wrap bare expressions so they are printed with
                // {{ ... }} braces even below the if statement
                const expr = this.matchExpression();
                tokens.expect(Types.EXPRESSION_END);
                attributes.push(
                    copyLoc(new n.PrintExpressionStatement(expr), expr)
                );
                continue;
            }
            attributes.push(this.matchAttribute(tokens));
        }
    }
